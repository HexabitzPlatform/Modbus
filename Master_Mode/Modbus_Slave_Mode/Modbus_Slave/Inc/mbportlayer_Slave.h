...
#ifndef _MB_PORT_LAYER_SLAVE_H
#define _MB_PORT_LAYER_SLAVE_H
#include "main.h"
...
#if defined( DOXYGEN) || \
( defined( MBP_SUPPORT_FRAME_ANALYZE_STAT_API ) && ( MBP_SUPPORT_FRAME_ANALYZE_STAT_API == 1 ) )
/*! \brief Provides a callback function for analyzing a frame. It is provided for the case
 *   when the stack does not analyze the frame by itself.
 *
 * If MBS_SERIAL_DROPFRAME_TIMEOUT_CB is set the stack only processes frames which are
 * targeted for itself. In this case the statistics and frame analyzer function do work
 * incorrectly. This function is provided to allow offline analysis of the frames in this case.
 * 
 * It is currently only implemented for RTU. If the frame timeout function returns the value
 * FALSE then the stack has not processed the frame. In this case the buffer can be stored
 * offline and later be passed to this function to correct the statistics.
 *
 * \param xHdl The MODBUS handle. This function is passed to the serial init functions and 
 *   the supplied handle of that function shall be used for further calls.
 * \param pubBuffer The serial data frame received without any prior processing.
 * \param usLength The length of the serial buffer.
 */
typedef void ( *pvMBPFrameAnalyzeStatCB )( xMBHandle xHdl, const UBYTE *pubBuffer, USHORT usLength );
#endif
...
eMBErrorCode    eMBPSerialInit( xMBPSerialHandle * pxSerialHdl, UCHAR ucPort, ULONG ulBaudRate,
                                ...
                                , pvMBPFrameAnalyzeStatCB pvMBPFrameAnalyzeStatFN
#endif
                                );
